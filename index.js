...
const checkCertificate = async (ragamID) => {
    
    let main = document.querySelector('.main');
    main.innerHTML = '';

    try {
        user = await fetch("./ragam-users/" + MD5(document.getElementById("RagamID").value) + ".json").then((res) => {
            return res.json();
        });
    }
    catch(e) {
    ...
    for(i=0; i<user.length; i++) {
        for(j=0; j<user[i].events.length; j++) {
            if(user[i].events[j].hasCertificate) {
                ...
                addText("No Certificates for " + user[i].events[j].name);
            }
        }
        if(j === 0) {
